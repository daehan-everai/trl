# Smoke Run Guide (GOLD external teacher)

## Purpose
Run the GOLD external-teacher smoke training against the vLLM teacher endpoint and log rollouts to W&B.
...
## Smoke run command (current config)
```bash
nohup env HF_HOME=/workspace/.cache/huggingface \
  ...
  python scripts/run_gold_external_teacher_smoke.py \
  ...
  --log-rollouts \
  --log-rollouts-steps 50 \
  --rollouts-per-log 10 \
  > runs/gold-external-teacher-smoke/teacher_run.log 2>&1 & echo $!
```

## Monitoring
- Log file: `runs/gold-external-teacher-smoke/teacher_run.log`
- W&B: use the run URL printed in the log.
- Check for teacher request errors:
```bash
rg -n "Teacher endpoint error|prompt_len|positions_min|positions_max" runs/gold-external-teacher-smoke/teacher_run.log
```
